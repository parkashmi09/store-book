...
const MyBag = () => {
  ...
  const totalPrice = bagItems
    .reduce((total: any, item: any) => {
      const priceAfterDiscount = calculateFinalPrice(
  ...
  const totalShippingCharges = bagItems
    .reduce((total: any, item: any) => {
        ...
        <div className="text-sm breadcrumbs">
          <ul>
            ...
            <li>
              <a>
                <IoBagHandleOutline />{" "}
                <span className="ml-1 underline">MyBag</span>
              </a>
            </li>
          </ul>
        </div>
                  ...
                  <div key={key}>
                    <div className="grid grid-cols-3 border mt-4 max-sm:grid-cols-1 shadow-md rounded-xl items-center gap-8">
                      <div>
                        <img
                          src={item?.images[0]}
                          className="transition h-44 ml-auto mr-auto p-4 "
                          alt="bag item"
                        />
                      </div>
                      <div className="col-span-2 max-sm:p-2">
                          ...
                          <div>
                            {item.discount ? (
                              <p>
                                ...
                                <span className="text-gray-500 text-sm opacity-80 line-through">
                                  &#8377; {item.original_price}
                                </span>{" "}
                        ...
                        <div className="flex items-center flex-col max-sm:items-start lg:flex-row mt-2 gap-2">
                          <div className="flex items-center">
                            <p className="text-xs font-semibold mr-1">
                              Quantity
                            </p>
                            <button
                        ...
                        <button
                          className="btn mt-1 btn-outline btn-sm ml-auto"
                          onClick={() => {
                            if (Object.keys(user).length > 0) {
                ...
                <div className="card-body !p-2">
                  ...
                  {offer.discount ? (
                    <div className="grid grid-cols-2">
                      ...
                      <p className="justify-self-end">
                        ...
                        {(
                          totalPrice -
                          calculateFinalPrice(totalPrice, offer?.discount)
                        ).toFixed(0)}
                      </p>
                    </div>
                  ) : null}
                  ...
                  {Object.keys(user).length > 0 ? (
                    <div className="grid border   p-2 rounded-xl bg-gray-100 grid-cols-2">
                      ...
                      <label
                        htmlFor="drawerPromo"
                        className="text-blue-600 justify-self-end"
                      >
                        Select
                      </label>
                      {offer.discount ? (
                        <p className="text-blue-600">1 offer applied</p>
                      ) : null}
                      {offer.discount ? (
                        <button
                          onClick={() => setOffer("")}
                          className="justify-self-end btn btn-xs  btn-outline"
                        >
                          Remove code
                        </button>
                      ) : null}
                  ...
                  {Object.keys(user).length > 0 ? (
                    tab == 1 ? (
                      <button
                        onClick={() => (
                          setTab(2), sessionStorage.setItem("settingtab", "2")
                        )}
                        className="btn mt-2   bg-gradient-to-r from-violet-600 to-indigo-600 text-white justify-self-end"
                      >
                        Place Order
                      </button>
                    ) : tab == 2 ? (
                    ...
                    <button
                      ...
                      className="btn mt-2  cursor-pointer bg-gradient-to-r from-violet-600 to-indigo-600 text-white justify-self-end"
                    >
                      Place Order
                    </button>
                  )}
      ...
      >
        <div className="grid grid-cols-2 max-sm:grid-cols-1 items-center mt-4  gap-2">
          <button
            ...
            onClick={() => removeBagItem(itemId)}
          >
            Remove
          </button>
          <button
            className="btn btn-neutral  btn-sm w-full"
            onClick={() => closeModal(dltRef)}
          >
            Cancel
          </button>
        </div>
              ...
              <li key={key}>
                <div className="card m-1 bg-base-100 c-shadow">
                  <div className="card-body !p-2">
                    ...
                    <button
                      onClick={() => (
                        setOffer(offerItm), drawerOffer.current.click()
                      )}
                      disabled={
                        offerItm.offerId === offer.offerId || !offerItm.status
                      }
                      className="btn btn-primary btn-sm"
                    >
                      {!offerItm.status
                        ? "Expired"
                        : offerItm.offerId === offer.offerId
                        ? "Applied"
                        : "Apply"}
                    </button>
                  </div>
